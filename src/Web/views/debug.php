...
<container>
   ...
   <nav>
      <ul>
         ...
         <li><a href="https://docs.rovota.com" target="_blank" rel="noreferrer">Documentation</a></li>
      </ul>
   </nav>
   ...
   <header>
      ...
      <?php if (isset($solution) && $solution instanceof Solution) { ?>
         <card class="solution">
            ...
            foreach ($solution->getDocumentationLinks() as $link_title => $link_url) {
               echo sprintf('<p><a href="%s">%s</a></p>', $link_url, $link_title);
            }
            ?>
         </card>
      <?php } ?>
   ...
   <main>
      ...
      <card class="stack">
         ...
         <preview>
            <heading>
               <?= str_replace('\\', '\\<wbr>', $throwable->getFile())?><small>:<?=$throwable->getLine()?></small>
            </heading>
            <file>
               <table>
                  <tr class="empty">
                  ...
                  <?php foreach ($snippet as $number => $content) {
                     $number++; $class = '';
